...
- Dynamic track info reading
- FIXED: Position INCREASES as you progress (0 @ max_position)
"""
...
class MarioKartEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.track_info_received = False
        ...
        print(f"  Track params will be read from game")
    ...
    def _normalize_state(self, state: np.ndarray) -> np.ndarray:
        ...
        if not self.normalize_state or self.max_position is None:
            return state
        
        normalized = state.copy()
        normalized[0] = min(state[0] / (self.max_steps / 60.0), 1.0)  # time
        normalized[1] = state[1] / self.max_position  # position (0 to max_position)
    ...
    def _read_game_output(self):
        while self.reader_running and self.process and self.process.poll() is None:
            try:
                ...
                if line:
                    if line.startswith("TRACK_INFO|"):
                        self._parse_track_info(line)
                    elif line.startswith("RL_STATE|"):
                        state = self._parse_state(line)
                        if state is not None:
                            self.state_buffer.append(state)
                            self.last_state = state
                    elif "EPISODE_END" in line or "RACE_END" in line:
                        self.state_buffer.append(("END", line))
            except:
                pass
    
    def _parse_track_info(self, line: str):
        """Parse TRACK_INFO message from game"""
        try:
            parts = line.split("|")[1:]
            if len(parts) >= 1:
                max_gradient = int(parts[0])
                self.max_position = max_gradient
                if self.lap_check_threshold is None:
                    self.lap_check_threshold = int(max_gradient * 0.975)
                
                print(f"\n Track info received:")
                print(f"  Max position: {self.max_position}")
                print(f"  Lap check threshold: {self.lap_check_threshold}")
                print(f"  Position INCREASES as you progress (0 @ {self.max_position})")
                
                if ImprovedRacingRewardComputer and self.reward_computer is None:
                    self.reward_computer = ImprovedRacingRewardComputer(
                        max_position=self.max_position,
                        lap_check_threshold=self.lap_check_threshold
                    )
                    print(f"  Reward computer initialized\n")
                
                self.track_info_received = True
        except Exception as e:
            print(f"Warning: Failed to parse track info: {e}")
    ...
    def _start_game_once(self):
        ...
        env = os.environ.copy()
        # No need to set RL_MODE - the game always enables RL during races
        # env["RL_MODE"] = "1"  # REMOVED - not needed anymore
        ...
        for _ in range(max_wait):
            if self.track_info_received:
                break
            time.sleep(0.1)
        
        if not self.track_info_received:
            print("Warning: Track info not received, using defaults")
            if self.max_position is None:
                self.max_position = 419
            if self.lap_check_threshold is None:
                self.lap_check_threshold = 409
            if ImprovedRacingRewardComputer and self.reward_computer is None:
                self.reward_computer = ImprovedRacingRewardComputer(
                    max_position=self.max_position,
                    lap_check_threshold=self.lap_check_threshold
    ...
    def step(self, action: int):
        ...
        # Early termination if stuck near start
        if self.current_step > 1800 and self.max_position and int(state[1]) < self.max_position * 0.1:
            truncated = True
            reward -= 100
...
if __name__ == '__main__':
    ...
    print(f"Track max_position: {env.max_position}")
    print(f"Position should INCREASE as you progress")
