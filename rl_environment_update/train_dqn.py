...
class QNetwork(nn.Module):
    ...
    #Larger hidden_dim to learn combinations
    def __init__(self, state_dim=11, action_dim=9, hidden_dim=256):
        super(QNetwork, self).__init__()

        self.network = nn.Sequential(
            nn.Linear(state_dim, hidden_dim),
            # Added normalization to equate speed and position
...
class DQNAgent(RLAgentBase):
    ...
    def select_action(self, state, training=True, force_action=None):
        ...
        # If agents is stuck (speed is near 0), inact the agent to brake and turn right (Edited by Camilla)
        speed = state[6]
        if speed < 0.05 and training and random.random() < 0.3:
            return 2, 0.0, None
    ...
    def _train_step(self):
        ...
        # Target Q-values
        with torch.no_grad():
            # Chooses best action from the Online Network then evaulates the action value using the target network (Edited by Camilla)
