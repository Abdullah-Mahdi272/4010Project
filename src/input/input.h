...
class Input {
   private:
    ...
    // RL input state
    static int rlAccel, rlBrake, rlLeft, rlRight;
    static bool rlInputActive;
   ...
   public:
    ...
    static inline bool held(const Key action) {
        // Check RL input first
        if (rlInputActive) {
            switch (action) {
                case Key::ACCELERATE: return rlAccel > 0;
                case Key::BRAKE: return rlBrake > 0;
                case Key::TURN_LEFT: return rlLeft > 0;
                case Key::TURN_RIGHT: return rlRight > 0;
                default: break;
            }
        }
    ...
    // RL input methods
    static void setRLInput(int accel, int brake, int left, int right);
