...
void StateRace::init() {
    // FORCE UNBUFFERED STDOUT/STDERR FOR RL MODE
    ...
    // Always enable RL mode during races (no environment variable needed)
    // This allows normal menu navigation while supporting RL control during race
    rlMode = true;

    std::cout << "*** RL COMMUNICATION ENABLED ***" << std::endl;
    std::cerr << "*** RL COMMUNICATION ENABLED ***" << std::endl;
    fflush(stdout);
    fflush(stderr);
    ...
    std::cout << "RL Agent initialized with initial state output" << std::endl;
    std::cerr << "RL Agent initialized with initial state output" << std::endl;
    fflush(stdout);
    fflush(stderr);
}
