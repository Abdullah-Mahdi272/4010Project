...
void StateRaceManager::init(const float _speedMultiplier,
    ...
    courseLoaded = false;      // Track that course needs to be loaded
...
bool StateRaceManager::update(const sf::Time &) {
    switch (currentState) {
        ...
        case RaceState::RACING: {
            ...
            // Only load course/audio if not already loaded (first race only)
            if (!courseLoaded) {
            ...
            game.pushState(StatePtr(
                new StateRace(game, drivers[(unsigned int)selectedPlayer],
                              drivers, positions)));
#ifndef NO_ANIMATIONS
            // ONLY do animation/AI loading on first race, skip on auto-restart
            if (!autoRestartEnabled || !courseLoaded) {
